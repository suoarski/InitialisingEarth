...
class GosplManager:
    ...
    def getDomainString(self, backwards=False):
        ...
        time = self.earth.startTime
        if backwards:
            npdataFormat = self.npdataBackFormat
            time = self.earth.endTime
    ...
    def createDomainNPdataFile(self):
        ...
        fileName = self.npdataFormat.format(self.npzFilesDirectory, self.earth.startTime)
        ...
        fileName = self.npdataBackFormat.format(self.npzFilesDirectory, self.earth.endTime)
        np.savez_compressed(fileName, v=self.icosphereXYZ, c=self.icoCells, n=neighbours.astype(int), z=heights)
    ...
    def createGosplDataMesh(self, gosplFilenamePattern, iteration):
    ...
    def animateGosplOutput(self,
        ...
        earthMesh = self.createGosplDataMesh(fileNamePattern, 0)
        ...
        for i in range(numOfFiles-1):
            newMesh = self.createGosplDataMesh(fileNamePattern, i+1)
